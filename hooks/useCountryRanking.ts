...
export const useCountryRanking = () => {
  const [rankingData, setRankingData] = useState<CountryRankingUser[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const { userProfile } = useAuth();

  const loadCountryRanking = async () => {
    if (!userProfile?.uid) {
      return;
    }
    ...
    try {
      setError(null);
      console.log('🌍 Chargement du ranking pays instantané...');
      ...
      const usersWithCountries: CountryRankingUser[] = [];

      // Mapping from country name to ISO code for normalization
      ...
      // Pour chaque utilisateur, récupérer ses informations et compter ses pays
      for (const userId of usersToRank) {
        try {
          ...
          // Extraire les pays uniques depuis les villes visitées
          const uniqueCountries = new Set<string>();
          
          visitedCities.forEach((city: any) => {
            if (city.country && city.beenThere) {
              // Normaliser le nom du pays
              let countryName = city.country.trim();
              
              // Si c'est un code ISO (2 lettres), on le convertit en nom complet
      ...
      // Trier par nombre de pays visités (décroissant)
      usersWithCountries.sort((a, b) => b.countriesVisited - a.countriesVisited);
      
      // Assigner les rangs
      let currentRank = 1;
      for (let i = 0; i < usersWithCountries.length; i++) {
        if (i > 0 && usersWithCountries[i].countriesVisited < usersWithCountries[i - 1].countriesVisited) {
          currentRank = i + 1;
        }
        usersWithCountries[i].rank = currentRank;
      }
      
      console.log(`✅ Country ranking chargé instantanément: ${usersWithCountries.length} utilisateurs`);
      setRankingData(usersWithCountries);
    } catch (err) {
      setError('Erreur lors du chargement du classement pays');
      console.error('Erreur country ranking:', err);
    }
  };

  // Rafraîchir le classement
  const refreshCountryRanking = async () => {
  ...
  return {
    ...
    loading,
    error,
    refreshRanking: refreshCountryRanking
  };
};
